...
void parseCommands(char  *ptrbufferReception){
	...
	if(strcmp(cmd, "help") == 0){
		...
		usart_WriteMsg(&usart2Comm, "Reinicio del sistema \n");
	}

	else if (strcmp(cmd, "Dir") == 0) {

		if (firstParameter == 1 && secondParameter >0){
			void forwardMove(uint8_t secondParameter);
		}
		else if (firstParameter == 0 && secondParameter >0){
			void backwardMove(uint8_t secondParameter);
		}
			...
			if (firstParameter > 0) {
				...
				updateDutyCycle(&PWM_R,firstParameter);
				sprintf(bufferMsg,"Velocidad actualizada %.2f s",firstParameter);
				usart_WriteMsg(&usart2Comm, bufferMsg);
			}
			else{
				writeMsg(&usart2Comm, "Porcentaje debe ser positivo.\n Ingresa \"help @\" para ver la lista de comandos.\n");
