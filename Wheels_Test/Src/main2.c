...
//	forwardMove(1);
	...
	while(1){
		...
		rawData(&imuHandler, rawGyro, dataTypeGyro);
		rawData(&imuHandler, rawTemp, dataTypeTemp);

		readData(rawAccel, accel ,dataTypeAccel,MPU6050_RANGE_2_G);
		readData(rawGyro, gyro,dataTypeGyro,MPU6050_RANGE_250_DEG);
		//temp = readData(rawTemp ,dataTypeTemp);

		for(uint32_t counter = 0; counter<30000;counter++){
...
void initSystem(void){
	...
	imuSDA.pinConfig.GPIO_PinPuPdControl		= GPIO_PUPDR_PULLUP;
	...
	imuHandler.modeI2C_FM	= I2C_MODE_SM_SPEED_100KHz_100MHz;
	imuHandler.maxI2C_FM	= I2C_MAX_RISE_TIME_FM_100MHz;
